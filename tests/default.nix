## Ardos Packer 2 Test Runner/Loader
{
  ...
  mkTestBaseContext = buildPlatform: targetPlatform: let
    system = buildPlatform.linuxTriple;
    target = targetPlatform.ardosTriple;
    ap2Instance = import ./fixtures/instance.nix {
  ...
  runUnitTestsForTarget = buildPlatform: targetPlatform: let
  ...
  mkIntegrationTest = ctx: test: let
    externalMappings =
    ...
    sysroot = ctx'.ap2Instance.sysroot {
  ...
  runIntegrationTestsForTarget = buildPlatform: targetPlatform: let
    ctx =
      mkTestBaseContext
      buildPlatform
      targetPlatform;
  ...
  runE2ETestsForTarget = buildPlatform: targetPlatform: let
    ctx = mkTestBaseContext buildPlatform targetPlatform;
  in
    lib.foldl'
    lib.recursiveUpdate
    ...
    (
      map (
        testFile: let
          spec =
            if builtins.isFunction testFile
            then testFile {}
            else testFile;
          targetTriple = targetPlatform.config;
          ...
          sysroot = instance.sysroot {
            ...
            includePackages = let
              pkgs = spec.packages or [];
            in
              if builtins.isFunction pkgs
              then pkgs instance.crossPkgs
              else pkgs;
          };
