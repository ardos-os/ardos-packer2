...
1. Registers `populate-map.rs` in configure/build hooks so link steps can see
   dependency runtime mappings before compilation starts. Also reads the current
   package's own layout from `ARDOS_CURRENT_PACKAGE_LAYOUT` for self-dependency.
1. Registers `generate-layout.rs` before fixup to create fallback empty layout
...
- `populate-map.rs`: scans Nix build flags and input metadata, follows relevant
  ...
  `ardos-layout` entries to the runtime map. Also processes the current
  package's `ARDOS_CURRENT_PACKAGE_LAYOUT` for folder-mapping self-dependency.
- `generate-layout.rs`: writes an empty `nix-support/ardos-layout` when a
  package has no explicit layout metadata.
