...
fn main() -> io::Result<()> {
    ...
    if is_debug() {
        eprintln!("[Ardos Setup] Created translation map at {}", temp_path.display());
    }

    println!("export ARDOS_LD_HOOK=\"{}\"", __ardos_ld_hook__);
    println!("export ARDOS_RUNTIME_MAP=\"{}\"", temp_path.display());
    eprintln!("[Ardos Setup] early-init exported: ARDOS_LD_HOOK={} ARDOS_RUNTIME_MAP={} ", __ardos_ld_hook__, temp_path.display());
